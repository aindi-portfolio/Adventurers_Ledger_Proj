...
const ItemCard = ({ item }) => {
  ...
  return (
    <div className="item bg-yellow-100 p-4 rounded shadow w-64">
      ...
      <button onClick={() => handleBuyOrSell('buy', item)} className=" bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600 transition-colors">Buy</button>
      <button onClick={() => handleBuyOrSell('sell', item)} className=" bg-yellow-300 text-yellow-800 px-4 py-2 rounded hover:bg-yellow-400 transition-colors">Sell</button>
      </div>
    </div>
  );
};
