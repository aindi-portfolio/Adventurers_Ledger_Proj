...
class Sign_Up(APIView):
    def post(self, request):
        ...
        new_user_account = UserAccount(data=data)
        if new_user_account.is_valid():
            new_user_account.save()
            token_object, _ = Token.objects.get_or_create(user=new_user_account.instance)
            login(request, new_user_account.instance)
...
class Login(APIView):
    def post(self, request):
        data = request.data.copy()  # {email, password}
        user_account = authenticate(username=data.get("email"), password=data.get("password"))
        if user_account:
